...
void ETH_CpuPortInit(void)
{
	uint16_t regval;

	/* Cpu port Auto-Negotiation disable */
	regval = ETH_ReadPHYRegister(2, 0);
	regval = regval & (0x2000 | 0x100);
	ETH_WritePHYRegister(2, 0, regval);
	regval = ETH_ReadPHYRegister(2, 0);
	regval |= 0x8000;
	ETH_WritePHYRegister(2, 0, regval);
	...
	/* Cpu port speed 100M */
	regval = ETH_ReadPHYRegister(2, 0);
	regval = (regval & (0x4000 | 0x100)) | 0x2000;
	ETH_WritePHYRegister(2, 0, regval);
	regval = ETH_ReadPHYRegister(2, 0);
	regval |= 0x8000;
	ETH_WritePHYRegister(2, 0, regval);

	/* Cpu port speed full-duplex */
	regval = ETH_ReadPHYRegister(2, 0);
	regval = (regval & (0x4000 | 0x2000 | 0x40)) | 0x100;	
	ETH_WritePHYRegister(2, 0, regval);
	regval = ETH_ReadPHYRegister(2, 0);
	regval |= 0x8000;
	ETH_WritePHYRegister(2, 0, regval);	
...
int M6095ReadPhyReg(u8 port_num, u8 regAddr, u16 *u16data)
{
	u16 orgPPU, ppuState;
	u16 retryCount;
	
	if(smi_getregfield(PHYADDR_GLOBAL, SW_REG_GLOBAL_CONTROL, 14, 1, &orgPPU) != SMI_DRV_SUCCESS)
		return ETH_ERROR;

	if(orgPPU) {
		if(smi_setregfield(PHYADDR_GLOBAL, SW_REG_GLOBAL_CONTROL, 14, 1, 0) != SMI_DRV_SUCCESS)
			return ETH_ERROR;
		
		for(retryCount = 0x100; retryCount > 0; retryCount--) {
			if(smi_getreg(PHYADDR_GLOBAL, SW_REG_GLOBAL_STATUS, &ppuState) != SMI_DRV_SUCCESS)
				return ETH_ERROR;			
	...
	if(smi_getreg(PHYADDR_PHY(port_num), regAddr, u16data) != SMI_DRV_SUCCESS)
		return ETH_ERROR;

	if(orgPPU) {
		if(smi_setregfield(PHYADDR_GLOBAL, SW_REG_GLOBAL_CONTROL, 14, 1, 1) != SMI_DRV_SUCCESS)
			return ETH_ERROR;
	}
...
int M6095WritePhyReg(u8 port_num, u8 regAddr, u16 u16data)
{
	u16 orgPPU, ppuState;
	u16 retryCount;
	...
	if(smi_setreg(PHYADDR_PHY(port_num), regAddr, u16data) != SMI_DRV_SUCCESS)
		return ETH_ERROR;
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
   ...
   ******************************************************************************/  
	if(smi_getreg(PHYADDR_GLOBAL, SW_REG_GLOBAL_CONTROL, &value) != SMI_DRV_SUCCESS)
		return ETH_ERROR;
	value |= 0x8000;
	if(smi_setreg(PHYADDR_GLOBAL, SW_REG_GLOBAL_CONTROL, value) != SMI_DRV_SUCCESS)
		return ETH_ERROR;
	timeout=0;
	do {
		timeout++;	
	} while ((ETH_ReadPHYRegister(PHYADDR_GLOBAL, SW_REG_GLOBAL_CONTROL) & 0x8000) && (timeout < PHY_READ_TO));
  ...
  smi_setregfield(PHYADDR_PORT(9), 0x01, 0, 6, 0x31);
  /* Setup PCS control register, force link up, and speed 1000M */
  smi_setregfield(PHYADDR_PORT(10), 0x01, 0, 6, 0x32);
  #endif /* BOARD_GV3S_HONUE_QM */
    
  #if BOARD_GE_EXT_22002EA
  /* Setup PCS control register, force link up, and speed 100M */
  smi_setregfield(PHYADDR_PORT(9), 0x01, 0, 6, 0x31);
  #endif /* BOARD_GE_EXT_22002EA */

  #if BOARD_GE220044MD
  /* Setup PCS control register, force link up, and speed 100M */
  smi_setregfield(PHYADDR_PORT(9), 0x01, 0, 6, 0x31);
  #endif /* BOARD_GE220044MD */
  ...
  {  
	...
	/* We wait for phy autoNego complete... */
    do
    {
      timeout++;
	...
	if(M6095ReadPhyReg(PHYAddress, SW_PHY_CONTROL_REG, &value) != ETH_SUCCESS)
		return ETH_ERROR;
	value &= 0x2100;
	value |= 0x1200;

	if(M6095WritePhyReg(PHYAddress, SW_PHY_CONTROL_REG, value) != ETH_SUCCESS)
		return ETH_ERROR;

#endif
#if 1
	/* We wait for phy linked status... */
	timeout=0;
    do
    {
      timeout++;
	...
	/* We wait for switch port linked status... */
	timeout=0;
    do
    {
      timeout++;
	} while (!((value = ETH_ReadPHYRegister(0x10+PHYAddress, 0)) & 0x0800) && (timeout < PHY_READ_TO));
    ...
    RegValue = ETH_ReadPHYRegister(0x10+PHYAddress, 0x00);

    /* Configure the MAC with the Duplex Mode fixed by the auto-negotiation process */
    if((RegValue & 0x0400) != (uint32_t)RESET)
    {
    ...
    else
    {
	  ...
	  //printf("Warning: Ethernet working at Half-duplex\r\n");
    ...
    if((RegValue & 0x0300)==0)
    {  
      /* Set Ethernet speed to 10M following the auto-negotiation */    
	  //printf("Warning: Ethernet speed is 10M\r\n");
...
#if 0  
  /* Set PPU Enable */
  if(!(ETH_WritePHYRegister(0x1b, 0x04, 0x4081)))
  {
  ...
  do{
      timeout++;
      value = ETH_ReadPHYRegister(0x1b, 0x00);
      value = ((value >> 14) & 0x03);
  }while((value != 0x03) && (timeout < PHY_READ_TO));
...
uint16_t ETH_ReadPHYRegister2(uint16_t PHYAddress, uint16_t PHYReg, uint16_t *RegValue)
{
  uint32_t tmpreg = 0;     
  __IO uint32_t timeout = 0;
