			...
			*pxPreviousWakeTime = xTimeToWake;
			...
			if( xShouldDelay )
			{
				...
				if( xTimeToWake < xTickCount )
				{
					...
					overflow list. */
					pxList = ( xList * ) pxOverflowDelayedTaskList;
					pxListItem = ( xListItem * ) &( pxCurrentTCB->xGenericListItem );
				...
				else
				{
					...
					current block list. */
					pxList = ( xList * ) pxDelayedTaskList;
					pxListItem = ( xListItem * ) &( pxCurrentTCB->xGenericListItem );
					vListInsert( pxList, pxListItem );					
...
/*-----------------------------------------------------------*/
	...
	unsigned short usStackRemaining;
		...
		do
		{
			...
			sprintf( pcStatusString, ( char * ) "  %-16s%1c%9u%12u%7u\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber );
		...
		do
		{
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				if( pxNextTCB->ulRunTimeCounter == 0 )
				{
					...
					sprintf( pcStatsString, ( char * ) "  %-16s%10u%10u%%\r\n", pxNextTCB->pcTaskName, ( unsigned int )0, ( unsigned int )0);
				}
				else
				{
					/* What percentage of the total run time as the task used?
					This will always be rounded down to the nearest integer. */
					if( pxNextTCB->ulRunTimeCounter > 42949672 )
						ulStatsAsPercentage = pxNextTCB->ulRunTimeCounter / ( ulTotalRunTime / 100UL );
					else
					...
					else
					{
						...
						sprintf( pcStatsString, ( char * ) "  %-16s%10u        <1%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter );
					}
				}
