...
GT_SEM osSemCreate(GT_SEM_BEGIN_STATE state)
{
	...
	if(state == GT_SEM_EMPTY) {
		xSemaphoreTake(xSemaphore,1);
	}
		
	return (GT_SEM)xSemaphore;
...
#elif defined(OS_FREERTOS)	
	vQueueDelete((xSemaphoreHandle)smid);
...
#elif defined(OS_FREERTOS)	
	if (timeOut == 0) {
		//xSemaphoreTake( (xSemaphoreHandle)smid, 0 );
		while( xSemaphoreTake( (xSemaphoreHandle)smid, portMAX_DELAY ) != pdTRUE ){}
	} else {
		if( xSemaphoreTake( (xSemaphoreHandle)smid, timeOut / portTICK_RATE_MS ) == pdTRUE )
			return GT_OK;
		else
			return GT_TIMEOUT;
	}
...
#elif defined(OS_FREERTOS)
	xSemaphoreGive( (xSemaphoreHandle)smid );
